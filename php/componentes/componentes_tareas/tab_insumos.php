                          ...
                          <td>
                              <div class="form-group mb-3" style="max-width: 100px;">
                                  <div class="input-group input-group-alternative"
                                      id="div_cant_usar<?php echo $ver[0] ?>">
                                      <input style="border-color: #fb6340;" id="cant_usar<?php echo $ver[0] ?>"
                                          type="number" class="form-control" placeholder="Cantidad a usar"
                                          autocomplete="off">
                                  </div>
                              </div>
                          </td>
 ...
 <div class="row">
      <div class="card bg-default shadow" style="width: 100%; ">
          <div class="table-responsive">
            ...
            unidad_de_medida.des_unm,terceros.ide_ter, 
            terceros.pno_ter, terceros.sno_ter, terceros.pap_ter, terceros.sap_ter
            FROM insumos, stock, registrar, compras, comprar, terceros, duenio, unidad_de_medida,  cultivos, act_cul, agroquimicos
            WHERE insumos.cod_ins=stock.cod_ins AND stock.cod_sto=registrar.cod_sto 
            AND registrar.cod_com=compras.cod_com AND compras.cod_com=comprar.cod_com
            AND comprar.ide_ter=terceros.ide_ter AND terceros.ide_ter=duenio.ide_ter
            AND unidad_de_medida.cod_unm=insumos.cod_unm AND terceros.ide_ter=act_cul.ide_ter 
             AND stock.can_sto != '0' AND insumos.cod_ins = agroquimicos.cod_ins
            AND act_cul.cod_cul=cultivos.cod_cul AND cultivos.cod_cul = '$cod_cul'
            AND  terceros.ide_ter LIKE '$like%'";
            $result=pg_query($conexion,$sql);
            while($ver=pg_fetch_row($result)){
          ...
          unidad_de_medida.des_unm,terceros.ide_ter, 
          terceros.pno_ter, terceros.sno_ter, terceros.pap_ter, terceros.sap_ter
          FROM insumos, stock, registrar, compras, comprar, terceros, socio, unidad_de_medida,  cultivos, act_cul, agroquimicos
          WHERE insumos.cod_ins=stock.cod_ins AND stock.cod_sto=registrar.cod_sto 
          AND registrar.cod_com=compras.cod_com AND compras.cod_com=comprar.cod_com
          AND comprar.ide_ter=terceros.ide_ter AND terceros.ide_ter=socio.ide_ter
          AND unidad_de_medida.cod_unm=insumos.cod_unm AND terceros.ide_ter=act_cul.ide_ter 
